...
void Calculation(void)
{
	...
	//--各部分の分割数
	int nbun_pole1,nbun_pole2,nbun_poleh;				//ダイポールの分割数
	...
	//--設計値関係
	double h_pole,h;
	double r_pole1,r_pole2,r_polec,r_poled,r_pole;		//ダイポールの長さ
	double wire_radius;									//ワイヤー半径
	double M_PI=3.14159265358979323846264338327950288;
	double radius;
	double x,y,aspect;									//アスペクト比
	...
	//　　　　　高さ0.625のとき(1.02570,0.08840)
	//			高さ0.1のとき(1.01577,0.10894)
	//			高さ0.125のとき(1.01099,0.12185)
	//========================================================

	r_polec = (1.0324 + 0.0001*k) * RAMDA0;		//周囲長
	r_poled = (0.2017 + 0.0001*n) * RAMDA0;			//横線の半分
	...
	//--全セグメント数設定
	n_pole = int(2.0*(nbun_pole1*4+4 + 2.0*nbun_pole2+2) + 2.0*nbun_poleh+2);
	NSEG = n_pole;								//全セグメント数
	...
	//========================================================
	PARA1 = r_pole/RAMDA0;
	PARA2 =	(r_pole-r_poled) /RAMDA0;
	PARA3 = h/RAMDA0;								
	PARA4 = (r_pole-r_poled) / r_pole;  //0.625のとき0.573383
	PARA5 =  USEF/FREQ0;
	PARA6 = r_polec/RAMDA0;
	PARA7 = r_poled/RAMDA0;
...
//------------------------------------------------反射板------------------------------------------------------------
	...
	for(j = 0; j < 2.0*(nbun_pole1+nbun_pole2+nbun_pole1+1+1+1); ++j){
		//--計算結果代入
		RX[i] = RX[j];											//始点位置x
		RY[i] = RY[j];											//始点位置y
		RZ[i] = -RZ[j];											//始点位置z
...
//-------------------------------------------------------------------------------------------------------------------
	...
	SEGN[8] = nbun_pole1 +1;
	SEGN[9] = nbun_pole1 +1;
	SEGN[10] = nbun_pole2 +1;
	SEGN[11] = nbun_pole1 +1;

	SEGN[12] = nbun_poleh*2 +2;
	...
	//					第１給電以外で制御を行う．
	//========================================================
	FEDP[0] = SEGN[0]*8-8 + SEGN[1]*4-4 + (SEGN[12]-2)/2 -1+1;
